...
export default function SearchModal(SearchModalProp: prop) {
    ...
    const { mutate: mutateSearch, mutateAsync: mutateAsyncSearch } = api.view.editSearch.useMutation({
        onSuccess: () => {
            utils.table.rowsAhead.reset({viewId: SearchModalProp.view.id, tableId: SearchModalProp.tableId})
        }
    })
    ...
    async function handleSearch(val: string) {
        setSearchVal(val);
        // call the api
        await mutateAsyncSearch({viewId: SearchModalProp.view.id, search: val})
    ...
    async function exit() {
        setSearchVal("");
        await mutateAsyncSearch({viewId: SearchModalProp.view.id, search: ""})
        utils.table.getViews.setData({tableId: SearchModalProp.tableId}, (prev) => {
            if (!prev) return prev
            return prev.map((view) => {
                if (view.id === SearchModalProp.view.id) {
    ...
    return (
        <div style={{gap: "3px", alignItems: "center", display: "flex", border: "solid rgba(222, 222, 222, 1) 0.5px", padding: "5px", borderRadius: "5px", left: `${SearchModalProp.position.left - 270}px`, top: `${SearchModalProp.position.top + 40}px`, zIndex: 900, width: "300px", height: "40px", background: "white", position: "fixed"}}>
            <input placeholder="Find in view" style={{ color: "grey", fontSize: "13px", height: "30px", width: "200px"}} type="text" value={searchVal} onChange={(e) => setSearchVal(e.target.value)} onBlur={(e) => handleSearch(e.target.value)}></input>
